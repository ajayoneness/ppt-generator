...
async function buildPPT(pptData, C, outputPath, screenshotImages = null, onProgress = null) {
  ...
  // Icon pool for modules/features (cycle through)
  const iconPool = [icons.brain, icons.camera, icons.cogs, icons.code,
                    icons.database, icons.server, icons.layer, icons.network,
                    icons.chip, icons.shield, icons.search, icons.flask];

  const layerIconPool = [icons.image, icons.cogs, icons.layer, icons.database,
                         icons.brain, icons.check];
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    sl.addImage({ data: icons.project, x: 4.5, y: 0.55, w: 1, h: 1 });
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    const statsIcons = [icons.project, icons.image, icons.chart, icons.rocket];
    (d.stats || []).forEach((st, i) => {
      ...
      sl.addImage({ data: statsIcons[i] || icons.star, x: 6.55, y: cy + 0.2, w: 0.45, h: 0.45 });
      sl.addText(st.num, {
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    (d.features || []).slice(0, 4).forEach((f, i) => {
      ...
      sl.addImage({ data: iconPool[i] || icons.brain, x: cx + 0.8, y: 1.95, w: 0.5, h: 0.5 });
      sl.addText(f.title, {
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    layers.forEach((l, i) => {
      ...
      sl.addImage({ data: layerIconPool[i] || icons.brain, x: lx + (layerW - 0.4) / 2, y: layerY + 0.2, w: 0.4, h: 0.4 });
      sl.addText(l.label, {
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    (d.modules || []).slice(0, 6).forEach((m, i) => {
      ...
      sl.addImage({ data: iconPool[i] || icons.brain, x: mx + 0.25, y: my + 0.25, w: 0.35, h: 0.35 });
      sl.addText(m.name, {
  ...
  // ───────────────────────────────────────────────────
  {
    ...
    sl.addImage({ data: icons.heart, x: 4.5, y: 1.2, w: 1.0, h: 1.0 });
